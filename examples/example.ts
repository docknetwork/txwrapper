...
/**
 * Entry point of the script. This script assumes a Dock node is running
...
async function main(): Promise<void> {
	...
	// Create Dock type registry.
	const registry = getRegistry({
		chainName: 'dock-dev-test-runtime',
		specName,
		specVersion,
		metadataRpc,
	});
	...
	console.log(
		...
		deriveAddress(alice.publicKey, registry.chainSS58 as number)
	);
	...
	const unsigned = methods.balances.transfer(
		...
		{
			address: deriveAddress(alice.publicKey, registry.chainSS58 as number),
			blockHash,
